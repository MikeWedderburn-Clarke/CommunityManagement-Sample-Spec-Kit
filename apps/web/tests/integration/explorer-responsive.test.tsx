...
describe("Explorer responsive layout", () => {
  it("renders 2-column grid layout structure", async () => {
    let container: HTMLElement;
    await act(async () => {
      ({ container } = render(<ExplorerShell events={[]} coordEvents={[]} />));
    });
  ...
  it("has mobile tab bar in DOM (hidden on desktop via CSS)", async () => {
    let container: HTMLElement;
    await act(async () => {
      ({ container } = render(<ExplorerShell events={[]} coordEvents={[]} />));
    });
  ...
  it("applies media query breakpoints via embedded style", async () => {
    let container: HTMLElement;
    await act(async () => {
      ({ container } = render(<ExplorerShell events={[]} coordEvents={[]} />));
    });
  ...
  it("includes touch-target tab styles", async () => {
    let container: HTMLElement;
    await act(async () => {
      ({ container } = render(<ExplorerShell events={[]} coordEvents={[]} />));
    });
